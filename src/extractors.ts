  ...
  const extractBlogPostData = (tag: string) => (link: string): Promise<void | PostEntry> => {
    return requester
      ...
      .then(({ data: blogPostHtml }) => {
        ...
        const postId = UrlBuilder.extractPostIdFromLink(link)
        console.log(`Successfully fetched data from ${link}`);
        return { title, date, content, tag,postId };
      })
      .catch((e) => {
        console.log(`Failed to fetch data from ${link}`,e);
      });
  };
